      ...
      message
        ...
        .then(() => {
          ...
          CREATE_REACT_MESSAGE({
            emojiId, roleId, guildId, categoryId, isCustomMessage, channelId,
...
export const updateReactMessages = async (
  ...
  try {
    ...
    if (!reactMessage) return log.debug(`No react messages exist with category[${categoryId}]`);

    const channel = await client.channels.fetch(reactMessage.channelId);
    if (!channel?.isText()) return log.debug(`Guild[${reactMessage.guildId}] apparently does not have channel[${reactMessage.channelId}]`);
    ...
    if (!reactMessage.isCustomMessage)
      await message
        .edit({ embeds: [embed] })
        .catch(() => {
          log.error(`Failed to edit message[${reactMessage.messageId}] with updated react role embed in guild[${reactMessage.guildId}]`);
        });
