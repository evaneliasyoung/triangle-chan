...
export namespace Random {
  ...
  export function number(min: any, max: any): number {
    return typeof min === 'number' ? typeof max === 'number' ? random() * (max - min) + min : random() * min : random();
  }
  ...
  export function pick<T>(...iterable: T[]): T { return iterable[Random.integer(iterable.length)]; }
