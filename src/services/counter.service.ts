...
export default class CounterService {
  getMembers = async (guild: Guild) => await guild.members.fetch({ withPresences: true });

  getOnlineMembers = (members: Collection<string, GuildMember>) =>
    members.filter(
      ({ presence }) => ['online', 'idle', 'dnd', 'invisible'].includes(presence?.status ?? '')
    );
  ...
  getRoledMembers = (members: Collection<string, GuildMember>, roleId: string) =>
    members.filter(
      ({ roles }) => !!roles.cache.find(role => role.id === roleId)
    );

  getCount = (members: Collection<string, GuildMember>, { name, roleId, type }: Counter) =>
    (() => {
      switch (type) {
        case ECounterType.total: return members;
        case ECounterType.online: return this.getOnlineMembers(members);
        case ECounterType.boost: return this.getPremiumMembers(members);
  ...
  handleGuild = async (guild: Guild) => {
    ...
    await Promise
      .all(counters.map(this.handleCounter.bind(this, guild, members)))
