...
import {
  dateToISO,
  dateToStringAndDuration,
  expandDateToUTC,
} from '../utils/luxon/datetime.js';
...
export default class EmbedService {
  ...
  #getChannelName = (channel: AnyChannel) =>
    channel.type === 'DM'
      ? `DM with ${channel.recipient.username}`
      : 'name' in channel
      ? channel.name
      : 'None';

  #getChannelTopic = (channel: AnyChannel) =>
    channel.type === 'DM'
  ...
  reactRolesFormattedString = (reactRoles: ReactRole[]) =>
    reactRoles
  ...
  aboutServerEmbed = async (guild: Guild) => {
    ...
    const emojiList = Array(Math.min(30, numEmojis))
      .fill(0)
      .map((_e, i) => emojis.cache.at(i)!.toString());
    ...
    return new MessageEmbed({
      ...
      fields: [
        {
          name: 'Owner',
          value: (await guild.fetchOwner())?.displayName ?? '[REDACTED]',
          inline: true,
        },
        ...
        {
          name: 'Channel Count',
          value: channels.valueOf().size.toString(),
          inline: true,
        },
        {
          name: 'Role Count',
          value: roles.valueOf().size.toString(),
          inline: true,
        },
        {name: 'Locale', value: preferredLocale, inline: true},
        {
          name: 'Explicit Content Filter',
          value: toTitleCase(explicitContentFilter.replace('_', ' ')),
          inline: true,
        },
        {
          name: 'Verification Level',
          value: toTitleCase(verificationLevel.replace('_', ' ')),
          inline: true,
        },
        {
          name: 'Creation Time',
          value: dateToStringAndDuration(created),
          inline: false,
        },
  ...
  aboutChannelEmbed = (channel: AnyChannel) => {
    const {id, createdAt, type} = channel;
    ...
    return new MessageEmbed({
      ...
      fields: [
        ...
        {name: 'Topic', value: this.#getChannelTopic(channel), inline: true},
        {
          name: 'Creation Type',
          value: dateToStringAndDuration(expandDateToUTC(createdAt)),
          inline: true,
        },
      ],
      color: COLOR.BRLL,
  ...
  aboutUserEmbed = (member: User | GuildMember) => {
    const user = 'user' in member ? member.user : member;
    const joinedAt =
      'joinedAt' in member ? member.joinedAt ?? new Date() : new Date();
    ...
    return new MessageEmbed({
      ...
      fields: [
        ...
        },
        {
          ...
          value: dateToStringAndDuration(joined),
          inline: false,
        },
      ],
      color: COLOR.BRLL,
  ...
  aboutBotEmbed = (client: Client) =>
    new MessageEmbed({
      ...
      thumbnail: {
        url:
          client.user!.avatarURL()?.toString() ??
  ...
  counterEmbed = async (client: Client, counter: Counter) => {
    ...
    let description = `Emoji: ${emojiId}\nType: **${
      ['Total', 'Online', 'Boost', 'Role'][type - 1]
    }**`;
    if (roleId) {
