...
export abstract class CounterListCommand {
  ...
  @Slash('counter-list', { description: 'List all your counter and the purpose of each.' })
  async execute(interaction: CommandInteraction) {
    if (!interaction.guildId) return log.error(`GuildID did not exist on interaction.`);

    const counters = await GET_COUNTERS_BY_GUILD_ID(interaction.guildId)
      .catch(MessageWithErrorHandler(`Failed to get counters for guild[${interaction.guildId}]`));

    if (!counters || !counters.length) {
      log.debug(`Guild[${interaction.guildId}] did not have any counters.`);
      return await interaction
        .reply(`Hey! It appears that there aren't any counters for this server... however, if there ARE supposed to be some and you see this please wait a second and try again.`)
